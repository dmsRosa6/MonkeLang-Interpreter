...
func TestIntegerLiteralExpression(t *testing.T){
	...
	if !ok {
		t.Fatalf("exp not found *ast.IntegerLiteral. got=%T", stmt.Value)
	}
...
func testIntegerLiteral(t *testing.T, il ast.Expression, value int64) bool {
	...
	if(integ.Value != value){
		t.Errorf("integ.Value does not match value %d. got = %d", value, integ.Value)
	}

	if( integ.TokenLiteral() != fmt.Sprintf("%d", value) ){
		t.Errorf("integ.TokenLiteral does not match value %d. got = %s", value, integ.TokenLiteral())
	}
...
func TestParsingPrefixExpressions(t *testing.T){
	...
	for _, tt := range(prefixTests) {
		...
		if !ok { 
			t.Fatalf("program.Statetements[0] is not ast.ExpressionStatement. got= %T", program.Statements[0])
		}
